...
const NONFOOD_JOB_KEYWORDS = [
  ...
  // アパレル（衣料小売。飲食店ではない）
  'アパレル',
  // ホテル運営の求人（施設はホテルであり飲食店ではない。※ニュース側の「ホテル内レストラン開店」は
  // isNonFoodJob が hiring 限定のため影響しない）
  'ホテル',
  // 飲食店ではない業種（判断確認済み）。エヴァーブルースカイはホテル運営だが、
  // タイトルが「オープニングホテ…」で切れ「ホテル」に一致しないため社名で除外する
  '株式会社サーズ', '和幸株式会社', 'カネカ食品', '株式会社ジュンバタンメラ', 'エヴァーブルースカイ',
];
