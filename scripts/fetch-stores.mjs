...
// data/ 配下の県別JSONを更新するスクリプト。GitHub Actions から1日6回実行される想定。
...
const EXCLUDE_KEYWORDS = [
  ...
  'メンズエステ', 'パチンコ', 'パチスロ', '風俗',
  // モダンクラブ（キャバクラの婉曲表現。「キャストレディ」「フロアレディ」もこの業態特有の職種名）
  'モダンCLUB', 'モダンクラブ', 'キャストレディ', 'フロアレディ',
  ...
  '開催決定',
  // ｄメニューニュース等の地域ニュースアグリゲーター経由で混入する、飲食と無関係な
  // 地域一般ニュース（速報・気象・鳥獣目撃・郷土施設等）。ジャンル・エリアが検出
  // されないまま「オープニング」求人系クエリにGoogleニュースが緩く一致させてくる
  '停電', '爆発', '竜巻', '落雷', '突風', '危険動物', '放鳥', '郷土資料館', '明るくする運動',
];
...
const NONFOOD_JOB_KEYWORDS = [
  ...
  // アパレル（衣料小売。飲食店ではない）
  'アパレル',
  // 不動産営業（飲食店ではない。「センチュリー21」「宅建資格保有」等の求人が混入）
  '不動産', '宅建',
