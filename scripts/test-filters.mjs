...
const MUST_EXCLUDE = [
  ...
  // 2026-08-01 ユーザー報告（千葉に混入）。ｄメニューニュース経由の地域一般ニュースが
  // 「オープニング」求人系クエリにGoogleニュースが緩く一致させて混入していた
  '【速報】千葉県内３４０軒停電',
  '【速報】千葉県内、落雷や竜巻など激しい突風に注意　大気の状態、不安定に　気象庁',
  '千葉県庁がイエローに　社会を明るくする運動で',
  '【速報】千葉・市川のマンションで爆発か、２人搬送　窓ガラス割れ、周辺に被害も',
];
...
const MUST_EXCLUDE_JOBS = [
  ...
  // 2026-08-06 ユーザー報告（埼玉に混入）。不動産営業の求人（飲食店ではない）
  '株式会社キムラ オープニングスタッフ募集（不動産営業/「宅建資格保有×今年入社の特権」不動産営業 オー…）',
  '「センチュリー21」オープニングスタッフ募集（不動産/営業アシスタント/販売 オープニング募集 接客経験歓…）',
];
...
// 2026-08-01 ユーザー報告（ｄメニューニュース経由の地域一般ニュース混入）。
// 埼玉・神奈川の記事のためisPrefRelevantではなくhasExcludeKeywordで直接検証
check(hasExcludeKeyword('【危険動物】クマ幼獣目撃　22日夕、秩父・滝沢ダム'), '除外ワード: クマ目撃情報は除外される');
check(hasExcludeKeyword('埼玉県内初、関東でも2例目…コウノトリの放鳥　鴻巣「天空の里」で2羽　愛称は「晴」と「天」に　ケージにとどまる1羽…もう1羽は翼を広げ、勢いよく飛び出す'),
