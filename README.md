...
| `scripts/fetch-stores.mjs` | Googleニュース検索RSS（開店ニュース）＋求人ボックス（オープニングスタッフ求人）から収集し `data/stores.json` を更新 |
...
- **オープニング求人の収集**は求人ボックスの検索結果から取得（`KYUJINBOX_SEARCHES` で検索条件を編集）。
  ページ構造が変わって取得できなくなった場合も、他の収集は通常どおり動作する
